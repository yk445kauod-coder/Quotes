...
async function buildExportHtml(docData: DocumentData, settings: SettingsData): Promise<string> {
    ...
    const itemsHtml = docData.items.map((item, index) => `
        <tr>
            ...
            <td style="border: 1px solid #ccc; padding: 5px; white-space: pre-wrap;">${item.description || ''}</td>
            <td style="border: 1px solid #ccc; padding: 5px; text-align: center;">${item.unit}</td>
            <td style="border: 1px solid #ccc; padding: 5px; text-align: center;">${item.quantity}</td>
            <td style="border: 1px solid #ccc; padding: 5px; text-align: right;">${formatCurrency(item.price || 0)}</td>
            <td style="border: 1px solid #ccc; padding: 5px; text-align: right;">${formatCurrency((item.quantity || 0) * (item.price || 0))}</td>
        </tr>
    `).join('');
    ...
    const termsAndPaymentHtml = docData.docType === 'quote' ? `
        <div style="flex-grow: 1; padding-right: 20px;">
            <h3 style="font-weight: bold; margin-bottom: 5px;">الشروط:</h3>
            <div style="white-space: pre-wrap;">${docData.terms || ''}</div>
            <h3 style="font-weight: bold; margin-top: 15px; margin-bottom: 5px;">طريقة الدفع:</h3>
            <div style="white-space: pre-wrap;">${docData.paymentMethod || ''}</div>
        </div>
    ...
    return `
      <html>
        <head>
          <meta charset="UTF-8">
          <style>
            body { font-family: 'PT Sans', sans-serif; direction: rtl; font-size: 10pt; }
            @page { 
              size: A4 portrait; 
              margin: 20mm;
            ...
            .header-placeholder, .footer-placeholder { height: 120px; } /* Adjust to match header/footer */
